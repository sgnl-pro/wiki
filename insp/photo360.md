...
<span style="flex-grow: 1;"> <a href="/insp/photo360/updates" onclick="event.stopPropagation();" style="color: inherit; text-decoration: none;">🔄 Что нового (история изменений этого функционала)</a></span>
</summary>
...
Можно настроить автоматическое сбрасывание статуса с Выполнено на Требуется на любой день недели, чтобы фотомейкер при синхронизации с проектом получал точки, на которые требуется загрузить фотографии. Подробнее об этом [«Настройки проекта»](/insp/settings)   
...
>Ссылки расшариваются на конкретную фотографию для использования, например, специалистами стройконтроля в замечании или департаментом продаж для отправки клиенту фото360 из его квартиры во время строительства.{.is-info}
...
## Комментарии к фото {#comments}

Чтобы оставить комментарий на фотографии, откройте проект в разделе «Фото360», выберите точку и фотографию, затем нажмите «Добавить комментарий». Укажите место на фотографии и заполните форму комментария. В форме можно указать название, дату, ссылку, статус, уровень критичности и описание, а также добавить вложения.
...
Откройте отдельный раздел «Комментарии» в меню проекта. Здесь можно просматривать комментарии по проекту, открывать их карточки, создавать комментарий кнопкой добавления, искать по тексту и применять фильтры. В реестре видны номер, название, описание, критичность, расположение, количество вложений, автор, даты и статус комментария. Для комментария, привязанного к фотографии, ссылка в столбце «Расположение» ведёт к месту на фото.
...
Чтобы выгрузить комментарии в формате XLSX, нажмите кнопку экспорта в верхней части реестра. Удаление выбранных комментариев доступно администратору.
