...
> Описанные здесь функции сейчас доступны только **Суперадминистратору SIGNAL**
> {.is-warning}
...
![конструктор_с_этапами.png](/конструктор_с_этапами.png)
...
Удалённые группы и типы форм доступны в **Корзине** в нижней части панели со списком типов форм. Нажмите кнопку **Корзина**, чтобы перейти в режим просмотра удалённых объектов. В нём можно восстановить удалённую группу или тип формы. Повторное нажатие возвращает к обычному списку.
...
:----- | :-----
**Строка** | Ввод текста до 10000 символов по заданной, при необходимости, маске.
...
Поля, получающие данные из других объектов, объединены в меню **«Значение из источника»**:

Тип поля | Отображение при заполнении формы
:----- | :-----
**Форма** | Выбор одной или нескольких завершённых форм проекта определённого типа с заданными, при необходимости, условиями фильтрации.
**Замечание** | Выбор одного или нескольких замечаний проекта DOCS.
**Комплект согласования** | Выбор одного или нескольких комплектов согласования проекта DOCS.
**Передача** | Выбор одной или нескольких передач проекта DOCS.
**Вид работ** | Выбор одного или нескольких значений из справочника видов работ компании HUB.
**Материал** | Выбор одного или нескольких значений из справочника материалов компании HUB.
**НТД** | Выбор одного или нескольких значений из справочника нормативно-технической документации компании HUB.
**Контрагент** | Выбор одного или нескольких контрагентов компании.
**Пользователь** | Выбор одного или нескольких пользователей компании.

### Удалённые поля

На вкладке **Поля** нажмите кнопку с иконкой корзины, чтобы открыть список удалённых полей выбранного типа формы. В этом режиме поле можно восстановить. Повторное нажатие кнопки возвращает к актуальному списку полей.

## 3. Общие свойства полей
...
**Системное имя** | Нет | Уникальный идентификатор для использования в шаблонах генерируемых документов. Допускаются латинские буквы, цифры и символ `_`.
...
**Уникальное значение в рамках проекта** | - | Галочка доступна для всех типов полей. Если она установлена, одно и то же значение этого поля нельзя сохранить в двух формах одного типа в пределах проекта. Удалённые формы при проверке не учитываются.
...
## 4. Дополнительные свойства полей

Свойство | Типы полей | Описание
:----- | :----- | :-----
**Множественный выбор**  | `Список вариантов` `Файл` `Форма` `Вид работ` `Материал` `НТД` `Замечание` `Комплект согласования` `Передача` `Пользователь` `Контрагент` | Галочка, можно ли при заполнении формы задавать несколько значений или только одно.
**Уникальное значение по полному совпадению** | `Список вариантов` `Файл` `Форма` `Вид работ` `Материал` `НТД` `Замечание` `Комплект согласования` `Передача` `Пользователь` `Контрагент` | Настройка доступна, если включены **«Множественный выбор»** и **«Уникальное значение в рамках проекта»**. Если она включена, система считает повтором только полностью совпадающий набор выбранных значений. Если выключена, значения проверяются по частичному совпадению: сохранить форму нельзя, если хотя бы одно выбранное значение уже используется в другой форме этого типа в проекте.
**Выпадающий список** | `Список вариантов` | Если настройка включена, варианты отображаются при заполнении формы в раскрывающемся списке. Если выключена — непосредственно в форме в виде переключателей или флажков.
**Отображение в названии**  | `Строка` `Число` `Дата и время` `Диапазон дат` `Список вариантов` | Галочка, должно ли значение поля отображаться в названии заполненной формы. Порядок отображения полей соответствует конструктору.
...
**Источник**  | `Форма` | Один тип форм и, при необходимости, условия фильтрации по значениям его полей. Например, можно выводить только формы, у которых значение поля совпадает со значением поля текущей формы.

## 5. Этапы согласования

Для каждого типа формы можно настроить маршрут согласования — до 20 этапов, которые должны пройти формы при заполнении: один ответственный вносит одни данные, затем следующий проверяет и дополняет.
