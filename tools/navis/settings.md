...
> ***Примечание:*** Для команд [_“Завершить”, “Принять” и “Запланировать”_](https://wiki.sgnl.pro/ru/tools/navis/approval#statuses) цвет будет задаваться в свойство SIGNAL-Color. Для команд  “Комментарий” и “Замечание” цвет будет задаваться в свойство SIGNAL - IssueColor.
{.is-info}


-   В нижней части окна представлены допустимые названия статусов замечаний (Issue) для файлов из DOCS. Для каждого цвета укажите через запятую название статусов. При импорте замечаний из DOCS командой [_“Из DOCS”_](https://wiki.sgnl.pro/ru/tools/navis/issues#from_docs) элементы будут окрашиваться в цвета указанных статусов.
...
4\. Закройте настройки, сохранив их в нижней части окна. Затем выделите необходимые элементы и с помощью команды [_"ПОЛЬЗ"_](https://wiki.sgnl.pro/ru/tools/navis/calculation#custom) на вкладке Расчеты посмотрите указанную числовую характеристику
...
## Стандартные ПН {#standart_searchsets}
### Стандартные ПН {#standart_searchsets}
  
В настройке "Стандартные ПН" указываются определенные правила для создания поисковых наборов. Например, по созданным правилам Вы можете разбить информационную модель с помощью инструмента [_“Стандартные”_](https://wiki.sgnl.pro/ru/tools/navis/search-sets) по любым сценариям: типы систем, виды оборудования, диаметры труб, материалы и т.д.

 1. Нажмите на панели Настройки ➤ Стандартные ПН
...
2\. Для того чтобы добавить правило создания ПН нажмите "+" в правой верхней части окна и выберите одно из правил
...
-   ***В строке “Название”*** впишите название правила, которое будет отображаться в окне создания [_“Стандартные”_](https://wiki.sgnl.pro/ru/tools/navis/search-sets#standart)
...
По данному правилу с помощью инструмента [*_“Стандартные”_*](https://wiki.sgnl.pro/ru/tools/navis/search-sets#standart) создаются поисковые наборы в панели Наборы.
...
#### 2.2. Комплексное правило с условиями
...
#### 2.3. Правило с условным свойством

Данное правило позволяет разбить модель по [_“Условному свойству”_](https://wiki.sgnl.pro/ru/tools/navis/settings#conditional).
...
#### 2.4. Правило древовидного построения
...
## Условные свойства {#conditional}
### Условные свойства {#conditional}
  
Это набор свойств элементов, которые в разных моделях или в разных элементах обозначают одно и тоже условное свойство. Например, свойство “Уровень” в разных элементах может быть в “Объект-Базовый уровень” или в “Объект-Уровень”. Данная настройка используется в [_“Стандартные ПН”_](https://wiki.sgnl.pro/ru/tools/navis/search-sets#standart) при создании поисковых наборов, а также при [_экспорте_](https://wiki.sgnl.pro/ru/tools/navis/export-import) свойств.
...
## Группы наборов {#serchsets_group}
### Группы наборов {#serchsets_group}
  
Настройка позволяет добавить группы наборов с элементами, которые требуется проверить на пересечения с помощью команды [_Checker_](https://wiki.sgnl.pro/ru/tools/navis/checker).

Нажмите на панели Настройки ➤ Настройки ➤ Группы наборов

C помощью команды [_“Стандартные”_](https://wiki.sgnl.pro/ru/tools/navis/search-sets#standart) предварительно создайте поисковые наборы с элементами , которые требуется проверить на пересечения.
...
Настройка позволяет добавить Компании, осуществляющие отдельные виды работ на объекте, и указать их при использовании инструментов [_Завершить/Принять/Запланировать_](https://wiki.sgnl.pro/ru/tools/navis/approval#statuses).
