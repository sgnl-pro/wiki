...
**SIGNAL Viewer** поддерживает модели в форматах GLB, IFC, RVT, NWD и NWC после обработки файла.
...
Клавиша <kbd>Esc</kbd> последовательно отменяет текущее действие: первое нажатие закрывает активный инструмент, следующее снимает сечение, сохраняя выбранные элементы, а ещё одно снимает выбор. После выхода из режима **«Сечение по грани»** его повторное включение снова ожидает выбора поверхности.

При включённом сечении можно скрывать и изолировать элементы без полного пересчёта среза.
...
В SIGNAL DOCS есть возможность собрать сводную модель проекта. Со сводной моделью также доступны инструменты замечаний и все стандартные инструменты просмотра BIM-моделей. В сборки можно добавлять в том числе файлы FBX.
