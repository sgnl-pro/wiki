---
title: DOCS. Уведомления — История изменений
description:
published: true
date: 2026-09-23T07:34:49.000Z
tags: docs
editor: markdown
dateCreated: 2026-09-23T07:34:49.000Z
---

<summary style="font-size: 16px; color: #0D47A1; background: #E3F2FD; border-radius: 7px; border: 1px solid #64B5F6; display: flex; gap: 10px; padding: 5px 16px; display: block; margin-top: 10px;">
<span style="flex-grow: 1;"><a href="/docs/notifications" onclick="event.stopPropagation();" style="color: inherit; text-decoration: none;">См. также основную статью <b>DOCS. Уведомления</b></a></span>
</summary>

> Здесь перечислены основные изменения. Для получения подробностей нажмите на номер версии.
{.is-info}

**Обозначения**
:fire: — новые возможности
:sparkles: — доработки функционала
:hammer_and_wrench: — исправление ошибок

----
**[2026.16](/general/updates/2026-16)** ^31.08.2026^

:fire: В [профиле пользователя](/hub/user#notifications) добавили получение уведомлений SIGNAL в MAX. Общие настройки каналов применяются к отдельным событиям по умолчанию.

![chrome_kbu1xm5nkg.png](/chrome_kbu1xm5nkg.png)

![6f24d872-8bc1-4aee-a349-e709326c146c.png](/6f24d872-8bc1-4aee-a349-e709326c146c.png)

![2026.16_-_макс.png](/release_notes/2026.16_-_макс.png =900x)

----
**[2026.13](/general/updates/2026-13)** ^20.07.2026^

:sparkles: Добавили уведомление об открытии комплекта согласования ответственным за связанные замечания. Этот тип уведомления по умолчанию выключен и включается в профиле пользователя.

----
**[2026.9](/general/updates/2026-9)** ^21.05.2026^

:fire: Расширенные настройки событий, каналов и объединения уведомлений перенесли в [профиль HUB](/hub/user#notifications). В DOCS оставили включение уведомлений для текущего проекта и управление подписками на папки.

![chrome_lakprgntqe.png](/chrome_lakprgntqe.png)

![chrome_3jcfnw889i.png](/chrome_3jcfnw889i.png)

![chrome_t8snhkzvur.png](/chrome_t8snhkzvur.png)

----
**[2025.37](/general/updates/2025-37)** ^27.11.2025^

:sparkles: В текст уведомлений о замечаниях и согласованиях добавили информацию об ответственном.

----
**[2025.29](/general/updates/2025-29)** ^03.10.2025^

:fire: Реализовали повторное оповещение ответственного на текущем этапе [согласования](/docs/reviews).

----
**[2025.27](/general/updates/2025-27)** ^18.09.2025^

:sparkles: В оповещения по подпискам на папки добавили событие «Копирование файла».

----

> Информация о более ранних обновлениях доступна в закрытом Telegram-канале для пользователей SIGNAL. Для добавления [обращайтесь в поддержку](/general/support).
{.is-info}
