...
<div class="toc-grid">
  ...
  <div class="toc-card toc-green">
    ...
    <ul class="toc-list">
      ...
      <li><a href="#color_issue"><span class="toc-badge">2.2</span> Цвет пометок</a></li>
      <li><a href="#bim_viewer"><span class="toc-badge">2.3</span> BIM-вьювер</a></li>
      <li><a href="#tangl_client"><span class="toc-badge">2.4</span> Клиент Tangl</a></li>
      <li><a href="#simple_view"><span class="toc-badge">2.5</span> Простой просмотр</a></li>
    </ul>
  </div>
  ...
  <div class="toc-card toc-purple">
    <h4 class="toc-card-title"><a href="#masks">6. Маски (Документы)</a></h4>
    <ul class="toc-list">
      Создание правил для наименование файлов
    </ul>
  </div>
</div>  
  
----
>Чтобы зайти в **Настройки проекта** в интерфейсе SIGNAL DOCS, нажмите на свой **профиль** в правом верхнем углу → **Настройки.** Изменения сохраняются сразу. Все внесённые изменения влияют только на текущий проект (отображается в левом верхнем углу).{.is-info}
...
## 6. Маски (Документы)
###### 6. Маски (Документы) {#masks}
      
 >Маски позволяют задавать определенные правила написания имени документа в папке.{.is-info}
  
  Для того чтобы назначить маску на папку - Нажмите правой кнопкой мыши на нужную папку, далее **Настройки** и **Настроить атрибуты и маски**
      ![chrome_mk7ttbywvn.png](/chrome_mk7ttbywvn.png)
      В открывшемся окне, добавьте новую маску для папки.
      В поле **Название** впишите шаблон наименования, которое должно использоваться в этой папке. Текст который будет написан в поле - будет неизменным, а чтобы пользователи могли вписывать свои значения - используйте спец.символы:
      **?** - символ позволяющий вписать пользователю один символ *(Пример: если пользователь должен написать строго четырехзначное число, то напишите **????**)*
      *- символ позволяющий вписать пользователю любое количество символов. Используется чаще всего когда нужно вписать название тома или раздела.
  В поле **Формат** впишите формат документа, например **pdf**, **rvt**, **dwg** и т.д.
  Пример написания маски для папки АР в проекте SIGNAL
  
  ![chrome_hvxqsppfrx.png](/chrome_hvxqsppfrx.png)

  В Масках, также можно использовать Атрибуты с типами `Текст`, `Дата` и `Список`.
  Такие маски можно использовать наряду с обычными масками без атрибутов. Для настройки маски с атрибутами нажмите на карандаш:
