...
{.is-info}
  ...
  7 — **Папка для копирования** В данную папку будут сохраняться акты приема-передачипакета
  8 — **Получатель пакета** Выберите пользователя или роль по умолчанию, который будет являться получателем пакета. Данный пользователь будет получать уведомления о пакетах передачи.
## 5. Атрибуты
