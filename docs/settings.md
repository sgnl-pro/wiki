...
**"titlecomparisonOps"** - примеры операторов сравнения, применяемых в конфигураторе;
...
**displayProps** -  какие свойства показывать в Избранных свойствах, при выборе набора в модели. (перечисляются через запятую уникальные имена свойств, например "$Category", "$S_Work", "$S_Unit")
...
Например, ["Объект", "Категория", "$EQUAL", "Стены", "$OR"], ["Объект", "Категория", "$EQUAL", "Перекрытия"]
**children**: вложенные категории

  Сначала указывается уникальное имя свойства, например "Category", которое будет указываться в Наборах, а дальше идет описание свойства:
